...
f                = Range(-10,10)             # or v1 = Range(-5, 10.0) if Range.unit = "km/s"
...
telescope   = "alma_400m",
tuningRange = [238.85,239.150],           #  in GHz
#TuningBand  = 60.0,                                 # in km/s
aijMin      = 0.0,
eup         = [0.0, 1000.0],
kup         = ["*","*"],
template    = "Full CDMS",  # or "Full CDMS" or "Full JPL" or "Full VASTEL" etc...
moltags     = [41505], #41001 - CH3CN All Species #41505 Full CDMS
tmb2ta      = False,
isoUnique   = False,
plotTitle   = myName,
warning	    = True,
...
selectedLines  = {"1-10": f},         # or selectedLines       = {"1": v1, "2": v2, "3":v3, "4":v4},  if selection on a velocity range
...
fwhm 		= {'min':2.0,    'max':10.0,    'nstep':8,  'log_mode':False},
size 		= {'min':0.2, 'max':1.3,   'nstep':10,  'log_mode':False},
vlsr 		= {'min':0.0,   'max':0.0,   'nstep':1,  'log_mode':False},
...
userInputs.computeChi2MinUsingRG(comp_1)      #userInputs.computeChi2MinUsingRG(comp_1)   
...
bestLineModel = userInputs.plotBestModel(moltag = 41505, overSampling=3)
